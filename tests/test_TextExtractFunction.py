...
def test_image_to_string():
    result, scale = TextExtractFunctions.image_to_string_only(os.path.abspath("./tests/MenuIcon.png"), lang='eng')
    assert result=='Menu',"Test Failed : Image to string not matched"

def test_image_to_string_matched():
    result, match = TextExtractFunctions.image_to_string_matched(os.path.abspath("./tests/MenuIcon.png"), expected_text='Menu', lang='eng')
    assert result=='Menu',"Test Failed : Image to string matched with expected"

def test_image_to_string_matched2():
    finalresult, scaled_results = TextExtractFunctions.image_to_string_matched(os.path.abspath("./tests/MenuIcon.png"), expected_text='Menu',
                                                                           all_results=True)
